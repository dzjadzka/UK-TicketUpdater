...
describe('Admin observability endpoints', () => {
  ...
  it('returns recent errors per user', async () => {
    ...
    expect(response.body.data.errors[0]).toHaveProperty('user_id');
    expect(response.body.data.errors[0].status).not.toBe('success');
  });
  ...
  it('summarizes jobs over the requested window', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.data).toBeDefined();
    expect(response.body.data.summary.success).toBeGreaterThanOrEqual(1);
    expect(response.body.data.summary.error).toBeGreaterThanOrEqual(1);
  });
  ...
  it('exposes the last base ticket state', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.data).toBeDefined();
    expect(response.body.data.state.base_ticket_hash).toBe('hash-123');
